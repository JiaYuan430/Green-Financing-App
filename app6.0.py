...
with menu[1]:
    ...
    if input_type == "💡 Monthly electricity bill (RM)":
        ...
        if matched and category == "Solar":
            # recommend system size but clamp to house-type limits
            ...
            system_size_kw = None
            if category == "Solar" and system_min is not None:
                ...
                monthly_savings_default = 1000  # generic fallback
    ...
    if st.button("Export Report"):
        ...
        if export_format == "CSV":
            ...
            # Yearly table
            data_yearly = [["Year", "Cumulative Savings"]] + [[int(y), f"RM {s:,.2f}"] for y, s in zip(df_yearly["Year"], df_yearly["Yearly Cumulative Savings"])]
